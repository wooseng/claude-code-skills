...
- 优先使用简洁的代码风格，让编译器为单个实例的常量或变量推断类型，例如：`let str = ""`，必要时才指定特定类型，如 `CGFloat` 或 `Int16`
- 对对象生命周期使用延迟初始化
- 对于空数组和字典，使用类型注解
...
应删除未使用的（无效的）代码，如果相关代码有注释，也将其一起删除。但教程或书籍明确要求用户使用注释代码的情况除外。
...
- 如果方法中存在不易理解的代码块，需要在对应的地方增加注释来说明，比如根据不同状态码区分的不同逻辑分支
